...
class SDDFBNeck(nn.Module):
    def __init__(self, c2, c3, c4, c5, out_channels=256, num_heads=16, window_size=64):
        super().__init__()

        self.fine_flow = FineFlow(c2, c3, out_channels)
        self.context_flow = ContextFlow(c4, c5, out_channels)
        self.fine_to_context = CrossAttention(out_channels, num_heads, window_size)
        self.context_to_fine = CrossAttention(out_channels, num_heads, window_size)
    ...
    def forward(self, p2, p3, p4, p5):
        ...
        fine_enhanced = self.fine_to_context(fine_feat, context_feat)
        context_enhanced = self.context_to_fine(context_feat, fine_feat)
